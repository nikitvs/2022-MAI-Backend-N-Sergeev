...
from .Serializers import CategorySerializer, WarehouseSerializer
...
class WarehouseAPIView(APIView):
    ...
    def get(self, request):        
        lst = Warehouse.objects.all()
        return Response({'data': WarehouseSerializer(lst, many=True).data})
    ...
    def post(self, request):
        ...
        warehouse_new = Warehouse.objects.create(
            warehouse_text=request.data['warehouse_name'],
            warehouse_img=request.data['warehouse_image']
        )
        
        return Response({'data': WarehouseSerializer(warehouse_new).data})